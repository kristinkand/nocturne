<script lang="ts">
  ...
  // Loading state - true when we don't have chart data yet
  const isLoading = $derived(
    glucoseData.length === 0 && serverChartData === null
  );
</script>
      ...
      <CardContent class="p-2">
        <div class="h-[450px] flex items-center justify-center">
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            <p class="text-xs text-muted-foreground max-w-[300px]">
              {error instanceof Error
                ? error.message
                : "An error occurred while loading the glucose chart."}
            </p>
            <button
              class="px-4 py-2 text-sm bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors"
              onclick={reset}
            >
              Try Again
            </button>
