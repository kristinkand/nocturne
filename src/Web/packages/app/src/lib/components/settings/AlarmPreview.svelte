<script lang="ts">
  ...
  import type { AlarmProfileConfiguration, EmergencyContactConfig } from "$lib/types/alarm-profile";
  import { Volume2, VolumeX, Square, Phone, Mail, AlertCircle } from "lucide-svelte";
...
{#if showEmergencyOverlay}
  <div class="fixed inset-0 z-101 flex items-center justify-center pointer-events-auto bg-black/60 backdrop-blur-sm">
    <div class="bg-background border-2 border-destructive rounded-xl p-6 max-w-md mx-4 shadow-2xl">
      <div class="flex items-center gap-3 mb-4">
        <div class="p-2 bg-destructive/10 rounded-full">
          <AlertCircle class="h-6 w-6 text-destructive" />
        </div>
        <div>
          <h2 class="text-lg font-bold text-destructive">IN CASE OF EMERGENCY</h2>
          <p class="text-sm text-muted-foreground">Contact the following people:</p>
        </div>
      </div>
      ...
      <div class="flex gap-2">
        <Button class="flex-1" variant="destructive" onclick={() => stopPreview()}>
          Acknowledge Alarm
        </Button>
      </div>
    </div>
