<script lang="ts">
	...
	let analyses = $state(data.analyses);
	...
	// Filter state
	let filterPath = $state(data.filters.requestPath || '');
	let filterMethod = $state(data.filters.requestMethod || '');
	let filterMatch = $state(data.filters.overallMatch || '');
	...
	function getMatchTypeDisplay(matchType: number) {
		const types = [
			{ value: 0, label: 'Perfect', class: 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' },
			{ value: 1, label: 'Minor Diff', class: 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' },
			{ value: 2, label: 'Major Diff', class: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' },
			{ value: 3, label: 'Critical', class: 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' },
			{ value: 4, label: 'NS Missing', class: 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' },
			{ value: 5, label: 'Nocturne Missing', class: 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' },
			{ value: 6, label: 'Both Missing', class: 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' },
			{ value: 7, label: 'Error', class: 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }
		];
	...
	let filteredAnalyses = $derived(
		showCompatible ? analyses : analyses.filter((a) => !isCompatible(a.overallMatch))
	);
...
<div class="container mx-auto p-6 space-y-6">
	...
	<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div>
				...
				<p class="font-mono text-sm">{config.nightscoutUrl || 'Not configured'}</p>
			</div>
			<div>
	...
	<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
			<h3 class="text-sm text-gray-500 dark:text-gray-400 mb-2">Total Requests</h3>
			<p class="text-3xl font-bold">{metrics.totalRequests || 0}</p>
		</div>
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
			<h3 class="text-sm text-gray-500 dark:text-gray-400 mb-2">Compatibility Score</h3>
			<p class="text-3xl font-bold">{(metrics.compatibilityScore || 0).toFixed(1)}%</p>
		</div>
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
			<h3 class="text-sm text-gray-500 dark:text-gray-400 mb-2">Critical Issues</h3>
			<p class="text-3xl font-bold text-red-600">{metrics.criticalDifferences || 0}</p>
		</div>
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
			<h3 class="text-sm text-gray-500 dark:text-gray-400 mb-2">Avg Response Time</h3>
	...
	<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
		...
		<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
			<div>
				<label for="filterPath" class="block text-sm font-medium mb-1">Request Path</label>
				<input
					type="text"
					id="filterPath"
			...
			<div>
				<label for="filterMethod" class="block text-sm font-medium mb-1">Method</label>
				<select id="filterMethod" bind:value={filterMethod} class="w-full px-3 py-2 border rounded-md dark:bg-gray-700">
			...
			<div>
				<label for="filterMatch" class="block text-sm font-medium mb-1">Match Type</label>
				<select id="filterMatch" bind:value={filterMatch} class="w-full px-3 py-2 border rounded-md dark:bg-gray-700">
					<option value="">All</option>
		...
		<div class="mt-4">
			<label class="flex items-center gap-2">
				...
				<span class="text-sm">Show compatible requests (Perfect & Minor Differences)</span>
			</label>
		</div>
	</div>
	...
	<div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
		<div class="px-6 py-4 border-b dark:border-gray-700">
			<h2 class="text-xl font-semibold">
				Recent Requests ({filteredAnalyses.length}{showCompatible ? '' : ' incompatible'})
			</h2>
		</div>
				...
				<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
					{#each filteredAnalyses as analysis}
						...
						>
							...
							<td class="px-6 py-4 whitespace-nowrap text-sm">
								<span class="font-mono">{analysis.nightscoutStatusCode || 'N/A'}</span>
								{#if analysis.nightscoutStatusCode !== analysis.nocturneStatusCode}
									<span class="text-red-600">≠</span>
									<span class="font-mono">{analysis.nocturneStatusCode || 'N/A'}</span>
								{/if}
							</td>
							...
							<td class="px-6 py-4 whitespace-nowrap text-sm">
								{#if analysis.criticalDiscrepancyCount > 0}
									<span class="text-red-600 font-semibold"
										>{analysis.criticalDiscrepancyCount} critical</span
									>
								{:else if analysis.majorDiscrepancyCount > 0}
									<span class="text-yellow-600 font-semibold"
										>{analysis.majorDiscrepancyCount} major</span
									>
								{:else if analysis.minorDiscrepancyCount > 0}
									<span class="text-blue-600">{analysis.minorDiscrepancyCount} minor</span>
								{:else}
							...
							<td class="px-6 py-4 whitespace-nowrap text-sm font-mono">
								<div class="flex gap-2">
									<span title="Nightscout">NS: {formatDuration(analysis.nightscoutResponseTimeMs || 0)}</span>
									<span title="Nocturne">NC: {formatDuration(analysis.nocturneResponseTimeMs || 0)}</span>
								</div>
							</td>
						</tr>
					{:else}
						<tr>
							<td colspan="7" class="px-6 py-12 text-center text-gray-500">
								No analyses found. Make sure the compatibility proxy service is running and receiving traffic.
							</td>
						</tr>
					{/each}
				</tbody>
