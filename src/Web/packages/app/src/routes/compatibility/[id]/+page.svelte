<script lang="ts">
	...
	let analysis = $state(data.analysis);

	// Helper to get match type display
	function getMatchTypeDisplay(matchType: number) {
		const types = [
			{ value: 0, label: 'Perfect Match', class: 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' },
			{ value: 1, label: 'Minor Differences', class: 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' },
			{ value: 2, label: 'Major Differences', class: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' },
			{ value: 3, label: 'Critical Differences', class: 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' },
			{ value: 4, label: 'Nightscout Missing', class: 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-300' },
			...
			{ value: 7, label: 'Comparison Error', class: 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }
		];
	...
	function getSeverityDisplay(severity: number) {
		const severities = [
			{ value: 0, label: 'Minor', class: 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' },
			{ value: 1, label: 'Major', class: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' },
			{ value: 2, label: 'Critical', class: 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }
		];
	...
	let discrepanciesBySeverity = $derived({
		critical: analysis.discrepancies?.filter((d: any) => d.severity === 2) || [],
...
<div class="container mx-auto p-6 space-y-6">
	...
	<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
			...
			<div>
				...
				<span class="inline-block px-3 py-1 text-sm font-semibold rounded-full {matchType.class}">
					{matchType.label}
				</span>
			</div>
			<div>
		...
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
			...
			<div class="space-y-3">
				...
				<div class="pt-2 border-t dark:border-gray-700">
					...
					<span class="ml-2 {analysis.statusCodeMatch ? 'text-green-600' : 'text-red-600'} font-semibold">
		...
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
			...
			<div class="space-y-3">
				...
				{#if analysis.nightscoutResponseTimeMs && analysis.nocturneResponseTimeMs}
					{@const diff = analysis.nocturneResponseTimeMs - analysis.nightscoutResponseTimeMs}
	...
	{#if analysis.selectedResponseTarget}
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					<p class="text-sm text-gray-500 dark:text-gray-400">Selected Target</p>
						...
						{#each discrepanciesBySeverity.critical as disc}
							<div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-4 border border-red-200 dark:border-red-800">
								<div class="flex justify-between items-start mb-2">
									<div>
										<span class="font-semibold">{getDiscrepancyTypeDisplay(disc.discrepancyType)}</span>
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
									<div>
										<p class="text-xs text-gray-500 dark:text-gray-400">Nightscout Value</p>
										<p class="font-mono text-sm bg-gray-100 dark:bg-gray-800 p-2 rounded break-all">
											{disc.nightscoutValue || 'null'}
										</p>
									</div>
									<div>
										<p class="text-xs text-gray-500 dark:text-gray-400">Nocturne Value</p>
										<p class="font-mono text-sm bg-gray-100 dark:bg-gray-800 p-2 rounded break-all">
	...
	{#if analysis.errorMessage}
		<div class="bg-red-50 dark:bg-red-900/20 rounded-lg shadow p-6 border border-red-200 dark:border-red-800">
			...
			<p class="text-sm font-mono whitespace-pre-wrap">{analysis.errorMessage}</p>
		</div>
	{/if}
</div>
